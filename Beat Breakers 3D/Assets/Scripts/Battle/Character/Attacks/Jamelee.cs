...
public class Jamelee : MonoBehaviour
{
    ...
    public void Attack()
    {
		...
		if (direction == "up") {
        	...
        	{
            	if (enemypos.y >= currentpos.y - 2 && enemypos.y <= currentpos.y - 1)
            	{
					enemy.GetComponent<VanillaCharacter>().TakeDamage(tempDamage);
            	}
			} else if (enemypos.x == currentpos.x) 
			{
			...
			{
				if (enemypos.x <= currentpos.x + 2 && enemypos.x >= currentpos.x + 1)
				{
					enemy.GetComponent<VanillaCharacter>().TakeDamage(tempDamage);
				}
